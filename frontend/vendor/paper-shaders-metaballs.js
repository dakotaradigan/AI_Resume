/*!
 * @paper-design/shaders v0.0.79 (bundled subset: ShaderMount + Metaballs)
...
}`;function V(){let t=navigator.userAgent.toLowerCase();return t.includes("safari")&&!t.includes("chrome")&&!t.includes("android")}function T(t){let e=visualViewport?.scale??1,i=visualViewport?.width??window.innerWidth,o=window.innerWidth-t.documentElement.clientWidth,s=e*i+o,r=outerWidth/s,a=Math.round(100*r);return a%5===0?a/100:a===33?1/3:a===67?2/3:a===133?4/3:r}var Q={fit:"contain",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0};var C={none:0,contain:1,cover:2};var F=`
...
`;var d={maxColorCount:8,maxBallsCount:20},M=`#version 300 es
precision mediump float;
...
void main() {
  ...
  for (int i = 0; i < ${d.maxBallsCount}; i++) {
    if (i >= int(ceil(u_count))) break;

    float idxFract = float(i) / float(${d.maxBallsCount});

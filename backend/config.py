...
class Settings:
    ...
    # Hard per-client-IP daily cap on token-spending requests (chat + JD).
    # Survives cookie rotation, so no single IP can drain the global budget;
    # password-unlocked visitors bypass it. Only enforced when trust_proxy_headers
    # is true (else every visitor shares the proxy IP). 0 disables.
    per_ip_daily_limit: int = 40
...
def get_settings() -> Settings:
    ...
    return Settings(
        ...
        per_ip_daily_limit=_to_int(os.getenv("PER_IP_DAILY_LIMIT"), 40),
